...
def process(time, rdd):
    ...
    try:
        ...
        if(initial_run):
            ...
            preds=clus.predict(x_train)
            #print(preds)
            #score=accuracy_score(y_train,preds)
            #score=clus.score(x_train)
            #print(count," ",clus.cluster_centers_score)
            mbk_means_cluster_centers = np.sort(clus.cluster_centers_, axis = 0)
            mbk_means_labels = pairwise_distances_argmin(x_train, mbk_means_cluster_centers)
            print(mbk_means_labels)


        clus.partial_fit(x_train)
